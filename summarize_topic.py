...
def generate_summary(texts):
    # Combine texts into a single content string
    full_text = " ".join(texts)
    
    # Generate summary
    summary = summarizer(full_text, max_length=130, min_length=30, truncation=True, batch_size=1)
    
    return summary[0]['summary_text']


keyword = input("Search string: ")
...
# summarizer = pipeline("summarization", model="google-t5/t5-small", revision="d769bba")
summarizer = pipeline("summarization", model="google-t5/t5-small", revision="d769bba", framework="tf", device=0)  # Add `device=0` if using GPU
