...
def main():
    filepath = '/Users/sheldonmrampton/Documents/@Reference/Social media/Facebook/facebook-sheldonrampton-2024-04-27-pvGjU2dZ/your_facebook_activity/posts/your_posts__check_ins__photos_and_videos_1.json'  # Update this path to where your JSON file is stored
    data = load_json_file(filepath)
    entries = extract_entries(data)

    csvfile = open('posts.csv', 'w', newline='')
    csvwriter = csv.writer(
        csvfile, delimiter=',',
        quotechar='"', quoting=csv.QUOTE_MINIMAL
    )
    csvwriter.writerow([
        'Timestamp', 'Date', 'Url', 'URL title', 'URL error', 'Post'
    ])
    ...
    for entry in entries:
        print(f"Timestamp: {entry['timestamp']}, URL: {entry['url']}, Post: {entry['post']}")
        # print(f"{entry['url']}")
        title = ''
        error = ''
